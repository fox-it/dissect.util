...
class CpioInfo(tarfile.TarInfo):
    ...
    def fromtarfile(cls, tarfile: tarfile.TarFile) -> CpioInfo:
        ...
        if tarfile.format not in (
            ...
            tarfile.format = detect_header(cast("BinaryIO", tarfile.fileobj))  # type: ignore
        ...
        obj = cls.frombuf(buf, tarfile.encoding, tarfile.errors, format=tarfile.format)  # type: ignore
        obj.format = cast("int", tarfile.format)
...
def CpioFile(*args, **kwargs) -> tarfile.TarFile:  # noqa: N802
    ...
    return tarfile.TarFile(*args, **kwargs)
