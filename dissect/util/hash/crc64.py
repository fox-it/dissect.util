"""CRC-64/NVME checksum using reflected polynomial 0x9A6C9329AC4BC9B5."""
...
def update(crc: int, data: bytes) -> int:
    """Update CRC-64/NVME checksum with data.
    ...
    Returns:
        The updated CRC-64 checksum value.
    """
    crc = crc ^ _MASK
    for b in data:
        table_idx = (crc ^ b) & 0xFF
        crc = _TABLE[table_idx] ^ ((crc >> 8) & _MASK)
    return crc ^ _MASK


def crc64(data: bytes, value: int = 0) -> int:
    """Calculate CRC-64/NVME checksum of some data, with an optional initial value.
    ...
    Returns:
        The CRC-64 checksum.
    """
    return update(value, data) & _MASK
